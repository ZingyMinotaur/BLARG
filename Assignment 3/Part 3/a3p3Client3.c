...
//This method to let the client sleep for 1 second, then repeats loop.
void sleep1(){
  //Let user know the client will go to sleep. Then loop the sleep command 1 time printing out "please wait" every time.
  printf("%s", "Going to sleep for 1 second.\n");
  int count = 1;
  while(count <= 1){
    sleep(1);
    printf("Please wait...\n");
...
void loopcmd(){
  ...
  outFile.open("a3p3Client3.log", std::ios_base::app);
  outFile << "------------------------------------------------------" << std::endl;
...
int main(int argc, char **argv) 
{
 ...
 int loopcount = 0;
 //Run the command loop 15 times (as each command loop sleeps for 1 second) for a total
 //of a minimum of 15 seconds of runtime.
 while (loopcount < 15) {
	//Send the user command to the server.
		
	char *cmdList[3] = {"date", "ls -l junk.txt", "uname -a"};
 ...
 loopcount = loopcount + 1;
  sleep1();
 } //Closing bracket for while loop (up to 15 times as described above).

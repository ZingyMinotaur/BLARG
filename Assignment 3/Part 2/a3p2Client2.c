...
//This method to let the client sleep for 2 seconds, then repeats loop.
void sleep2(){
  //Let user know the client will go to sleep. Then loop the sleep command 2 times printing out "please wait" every time.
  printf("%s", "Going to sleep for 2 seconds.\n");
  int count = 1;
  while(count <= 2){
    sleep(1);
    printf("Please wait...\n");
...
*/
  ...
  outFile.open("a3p2Client2.log", std::ios_base::app);
  outFile << "------------------------------------------------------" << std::endl;
...
int main(int argc, char **argv) 
{
 ...
 int loopcount = 0;
 //Run the command loop 8 times (as each command loop sleeps for 2 seconds) for a total
 //of a minimum of 16 seconds of runtime.
 while (loopcount < 8) {
	//Send the user command to the server.
	//send the command: " date; ls -l junk.txt; rm -f junk.txt; ls -l junk.txt  "
	
	char *cmdList[4] = {"date", "ls -l junk.txt", "rm -f junk.txt", "ls -l junk.txt"};
	int cmdLength[4] = {4,14,14,14};
	int commandcount = 0;
	...
	//Else, get the result from the server.
		...
		//Call the sleep2 method. Mainly puts the client to sleep for 2 seconds, then repeats loop.
      	loopcmd();

 		} //Closing bracket for else statement in while loop
 	commandcount = commandcount + 1;
	} //Closing bracket for while loop to run the 4 given commands.

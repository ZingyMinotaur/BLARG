...
#define PARMPTR ((struct serverParm *) parmPtr)
            ...
            read(PARMPTR->connectionDesc,messageBuf,sizeof(messageBuf)-1)) > 0) 
    {
        ...
        //Open the server log file
        std::ofstream outFile;
 	    outFile.open("a3p2Server.log", std::ios_base::app);
        ...
        //If the errtemp file has something, get contents of errtemp (command failed)
        //Else, get the data from the temp file.
      /*
          int fp = fopen("errtemp.txt","r" );
          int size = 0;  
          if (NULL != fp) {
          fseek (fp, 0, SEEK_END);
          size = ftell(fp);
    ...
    // file is empty
        ...
        */  //  if (0 == size) {
        //errFile = "errtemp.txt";
              ...
              tempFile.close();
          //       printf("%s", "This temp successful!\n");
            }
            else{
              //Now get data from the errtemp file created from the runcmd method
              //This contains the result of hte client command,
         //     std::ifstream errFile ("errtemp.txt", std::ios::in);
              ...
              errFile.close();
       //       printf("%s", "This errror successful!\n");
